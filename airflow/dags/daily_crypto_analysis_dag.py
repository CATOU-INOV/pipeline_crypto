...
def check_data_availability(**context):
    ...
    try:
        ...
        if count < 1000:
            error_msg = f"Données insuffisantes : {count} < 1000 enregistrements"
...
default_args = {
    ...
    'retries': 3,  # 3 tentatives comme demandé dans le TD
    'retry_delay': timedelta(minutes=2),  # Délai initial
    'retry_exponential_backoff': True,  # Backoff exponentiel
    'max_retry_delay': timedelta(minutes=8),  # Max 8 minutes
    'execution_timeout': timedelta(minutes=30),  # Timeout par tâche
    'sla': timedelta(hours=2),  # SLA de 2 heures comme demandé
}

with DAG(
    'daily_crypto_analysis',
    default_args=default_args,
    description='Pipeline quotidien d\'analyse crypto - Calcul des stats et génération de rapports',
    schedule_interval='0 1 * * *',  # Tous les jours à 1h du matin
    start_date=days_ago(2),  # Commence il y a 2 jours pour avoir de l'historique
    catchup=False,
